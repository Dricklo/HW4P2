...
class ASRDataset(Dataset):
    def __init__(
        ...
        # Track normalization statistics when using global mean/variance.
        ...
        # Store FBANK directory path for loading numpy feature files per partition.
        
        # TODO: Get all feature files in the feature directory in sorted order  
        self.fbank_files = sorted(
            [file for file in os.listdir(self.fbank_dir) if file.endswith(".npy")]
        )
        # Sorting keeps features aligned with transcripts since filenames encode utterance ids.
        
        # TODO: Take subset
        subset_fraction  = float(self.config.get('subset', 1.0))
        subset_fraction  = min(max(subset_fraction, 0.0), 1.0)
        subset_size      = len(self.fbank_files) if subset_fraction >= 1.0 else (
            0 if subset_fraction == 0.0 else max(1, int(len(self.fbank_files) * subset_fraction))
        )
        self.fbank_files = self.fbank_files[:subset_size]
        # Respect the requested subset fraction so smaller debug datasets can be loaded quickly.
        ...
        if self.partition != "test-clean":
            ...
            # Companion directory for transcript numpy files.

            # TODO: Get all text files in the text directory in sorted order  
            self.text_files = sorted(
                [file for file in os.listdir(self.text_dir) if file.endswith(".npy")]
            )
        ...
        for i in tqdm(range(self.length)):
            ...
            # Load spectrogram features stored as numpy arrays for the utterance.

            # TODO: Truncate features to num_feats set by you in the config
            feat = feat[:self.config['num_feats'], :]
            # Keep only the requested number of Mel bins for downstream models.
            ...
            if self.partition != "test-clean":
                ...
                transcript_chars = np.load(transcript_path).tolist()
                transcript = ''.join(transcript_chars)
                # Convert stored character array back into a raw string transcript.
                ...
                # Shifted starts with SOS, golden ends with EOS for teacher forcing targets.
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Cast cached numpy features to float tensors for the model pipeline.
        ...
        if self.partition != "test-clean":
            ...
            # Convert stored python lists into tensors for downstream batching.
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Convert each utterance to (time, feat) so pad_sequence can batch across time.
        ...
        padded_feats = pad_sequence(batch_feats, batch_first=True, padding_value=float(self.pad_token)) # B x T x F
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.LongTensor([seq.shape[0] for seq in batch_shifted]) # B  
