...
def PadMask(padded_input, input_lengths):
    ...
    # Build timestep indices and compare against actual lengths
    batch_size, seq_len = padded_input.shape[:2] # get first two dimensions.
    device = padded_input.device # set input to be on same device as input tensor
    time_indices = torch.arange(seq_len, device=device).expand(batch_size, -1)
    valid_lengths = input_lengths.to(device).unsqueeze(1) # change dimension to N,1
    mask = time_indices >= valid_lengths # everything at /after sequence length marked true. 
    return mask
...
def CausalMask(padded_input):
    ...
    t_len = padded_input.shape[1]
    device = padded_input.device
    # Upper triangular (excluding diagonal) marks future tokens as masked
    mask = torch.triu(torch.ones((t_len, t_len), dtype=torch.bool, device=device), diagonal=1)
    return mask
