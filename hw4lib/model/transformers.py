...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dec_layers = nn.ModuleList(  # Stack identical masked decoder blocks
            [
        ...
        self.target_embedding = nn.Embedding(  # Convert token ids to dense vectors
            num_embeddings=num_classes,
            embedding_dim=d_model,
        )
        self.positional_encoding = PositionalEncoding(  # Adds deterministic positional info
            d_model=d_model,
            max_len=max_len,
        )
        self.final_linear = nn.Linear(  # Project decoder states back to vocab logits
            d_model,
            num_classes,
        )
        self.dropout = nn.Dropout(dropout)  # Shared dropout used after embeddings
        self.norm = nn.LayerNorm(d_model)  # Final Pre-LN normalization
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        if target_lengths is not None:
            pad_mask_dec = PadMask(padded_targets, target_lengths)  # Mask padded tokens
        
        # TODO: Create causal mask to prevent attending to future tokens on the same device as the input (use CausalMask)
        causal_mask = CausalMask(padded_targets)  # Prevent attention to future steps

        # TODO: Apply the embedding
        x = self.target_embedding(padded_targets)  # Embed discrete ids to vectors
        # TODO: Apply positional encoding
        x = self.positional_encoding(x)  # Inject absolute position information
        # TODO: Apply dropout 
        x = self.dropout(x)  # Regularize before transformer stack
        ...
        for i in range(self.num_layers):
            ...
            x, attention = self.dec_layers[i](  # Masked self-attention and FFN block
                x=x,
        ...
        seq_out = self.final_linear(x)  # Predict logits for each position
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        ctc_inputs = {
            'log_probs': ctc_logits.transpose(0, 1).contiguous(),
            'lengths': x_enc_lengths.to(dtype=torch.int32, device='cpu'),
        }
